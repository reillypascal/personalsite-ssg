...
For the images, I design SVGs (or edit existing ones, such as logos) in Inkscape, touch up and scale photos in GIMP and/or ImageMagick, and cite e.g., digital signal processing diagrams from papers or other discussions when relevant. Photos prior to 2025 were taken on an iPhone SE 2020 (without “Apple Intelligence” used), and photos after that are taken on a Pixel 8a running GrapheneOS. I do not have Google Gemini (or similar software) on my GrapheneOS installation — the only on-device image processing is via the default hardware settings and the [GrapheneOS camera app](https://github.com/GrapheneOS/Camera).
